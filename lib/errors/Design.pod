...
The primary goal behind the C<errors> module is to finally add error handling to Perl that people actually want to use.
...
=item * Export a try/catch interface.

=item * Provide a base class for error objects.

=item * Provide a hierarchy of standard/system error classes.
...
Python gives you an C<else> clause. Evidently, tryI<else is an idiom just like if>else.
...
Therefore, while the C<errors> module probably needs to coexist and be aware of Error.pm, it will not attempt to follow its exact syntax and semantics.

=head1 Exceptions.pm

There is a module called Exceptions to which the C<Exception> (non plural) is indexed. It is only at version 0.01 and has no documentation. It appears to be dead.
...
=item * Work with the authors/maintainers of Error.pm and autodie

=item * Look into how Perl 6 handles errors
